...
service_to_template = {
                       ...
                       # service, a device template with the name 'WinRM' 
...
try:
    ...
    if error is not None and len(error) > 0:
        ...
        self.logger.debug("Larry: data is not None ")
        # Rearrange
        ...
        for key in n.keys():
            ...
            self.logger.debug("Larry service_to_template name: %s -> %s" % (n[key], tmpl))
            if tmpl is not None:
                tid = get_template_id(dbc, tmpl)
                self.logger.ui_debug("Larry - tmpl to tid: %s -> %s" % (tmpl, tid))
                if tid:
                    try:
                        r = api.apply("/api/device/%s" % (self.did), "/api/device_template/%s" % (tid))
                    except Exception, err:
                        self.logger.debug("Larry - Error while applying templates to device: %s" % (err))
        ...
        self.logger.ui_debug("Larry - name: %s" % (name))
    ...
    self.logger.debug("Larry - Error: %s" % (err))
